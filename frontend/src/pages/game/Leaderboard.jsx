...
const API_BASE = 'http://127.0.0.1:8000/player';
...
export default function Leaderboard({
  ...
  const playerEntry = playerName ? top.find((p) => p.name === playerName) : null;
    ...
    <ModalOverlay>
      <ModalContent>
        <Header>
          ...
          <div>
            <button onClick={fetchTop} style={{ marginRight: 8, marginBottom: 15 }}>
              Refresh
            </button>
        ...
        <ScoreList>
          {top.length === 0 && <div style={{ color: '#666' }}>No scores yet.</div>}
          {top.map((p, idx) => {
            ...
            return (
              ...
              <ScoreListItem key={p.id ?? `${p.name}-${idx}`} $isYou={isYou}>
...
const ScoreListItem = styled.li`
  ...
  border-radius: 6px;
  margin-bottom: 6px;
  background: ${props => props.$isYou ? 'linear-gradient(90deg, #272525ff, #575654ff)' : 'transparent'};
  box-shadow: ${props => props.$isYou ? 'inset 0 0 0 1px rgba(0,0,0,0.04)' : 'none'};
`;
